...
#   Sibling promotion: if any sibling function in same PCI slot shows SmartNIC pattern,
#   promote the entire slot to SmartNIC (so eth6/eth7 are treated as same SmartNIC).
...
for dev in /sys/class/net/*; do
  ...
  # FIX: check dmidecode outside the [[ ... ]] test
  ...
  # SIBLING PROMOTION: if any sibling in same PCI slot is SmartNIC, promote this iface
  if [[ "$type" == "SuperNIC" && "$pci" != "N/A" ]]; then
    slot="${pci%.*}"   # drop function, e.g. 0000:61:00
    for sibpath in /sys/bus/pci/devices/"${slot}".*; do
      [[ -e "$sibpath" ]] || continue
      sibpci="$(basename "$sibpath")"
      # find iface for sibling pci
      sibiface="$(grep -m1 "^${sibpci}|" "$MAP_FILE" 2>/dev/null | cut -d'|' -f2 || true)"
      [[ -z "${sibiface:-}" ]] && continue
      # read sibling macs; handle missing tools quietly
