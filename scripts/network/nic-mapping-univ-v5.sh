#!/bin/bash
# nic-mapping-univ-v21.1.sh (vendor-neutral, PN-focused, fixed)
# - No Mellanox tools required
# - PN only (from PCI VPD / lspci "Part number" if present)
# - Firmware via ethtool -i
# - SmartNIC detection via heuristics (Cisco/Dell/Lenovo)
#
...
# Extract PN from PCI VPD (best-effort). Returns a PN-like line or empty.
pn_from_vpd() {
  local pci="$1"
  ...
  # Try to find a line containing PN / P/N / Part Number markers.
...
for dev in /sys/class/net/*; do
  ...
  # Type classification (vendor-neutral)
  type="Generic-NIC"
  if echo "$desc" | grep -qi "BlueField-3"; then
    type="SuperNIC"

    psid="$(sysread "/sys/class/net/$iface/phys_switch_id")"
    ppn="$(sysread "/sys/class/net/$iface/phys_port_name")"
    if [[ "$psid" != "N/A" && -n "$psid" ]] || [[ "$ppn" != "N/A" && -n "$ppn" ]]; then
      type="SmartNIC"
    fi

    if [[ "$type" == "SuperNIC" && "$pmac" != "N/A" && "$cmac" != "N/A" && "$cmac" != "$pmac" ]]; then
      type="SmartNIC"
    fi
  fi
  ...
  echo "$iface|$func|$parent_pci|$parent_iface|$cmac|$pmac|$sn|$pn|$fw|$stat|$type|$pci|$desc" >> "$TMP_DATA"
  count=$((count+1))
done
...
# --- 3) Print header ---
fmt="%-$(($w_iface+2))s %-$(($w_func+2))s %-$(($w_parent+2))s %-$(($w_piface+2))s %-$(($w_cmac+2))s %-$(($w_pmac+2))s %-$(($w_sn+2))s %-$(($w_pn+2))s %-$(($w_fw+2))s %-$(($w_stat+2))b %-$(($w_type+2))s %-$(($w_pci+2))s %-$(($w_desc))s\n"

printf "$fmt" \
  "IFACE" "FUNC" "PARENT PCI" "PARENT IFACE" "CURR MAC" "PERM MAC" "SERIAL" "PN" "FW" "STATUS" "TYPE" "PCI" "MODEL DESCRIPTION"

# --- 4) Print rows ---
while IFS='|' read -r iface func p_pci p_iface cmac pmac sn pn fw stat type pci desc; do
  if [[ "$stat" == "up" ]]; then s_disp="${GREEN}UP${NC}"; else s_disp="${RED}DOWN${NC}"; fi

  printf "%-$(($w_iface+2))s %-$(($w_func+2))s %-$(($w_parent+2))s %-$(($w_piface+2))s %-$(($w_cmac+2))s %-$(($w_pmac+2))s %-$(($w_sn+2))s %-$(($w_pn+2))s %-$(($w_fw+2))s " \
    "$iface" "$func" "$p_pci" "$p_iface" "$cmac" "$pmac" "$sn" "$pn" "$fw"
